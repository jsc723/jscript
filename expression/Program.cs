...

{
    class Program
    {
        ...
        static string __version__ = "1.1.0";
        static string __myname__ = "Sicheng Jiang";
        static string __myEmail__ = "jscjsc723723@163.com";
        static string __releaseTime__ = "2017.4.29";
        static public string[] mergerArray(string[] First, string[] Second)
        {
        ...
        static public string[] mergerArray(string[] Source, string str)
        {
        ...
        static public string[] mergerArray(string str ,string[] Source)
        {
            string[] result = new string[Source.Length + 1];
            result[0] = str;
        ...
        static public string[] subTokens(string[] src, int start, int len = 99999)
        {
            if (len < 1) len = 0;
            if (len == 99999) len = src.Length - start;
        ...
        {
            if (index < 0)
                return mergerArray(t, tokens);
            if (index > tokens.Length - 1)
                return mergerArray(tokens, t);
            return mergerArray(mergerArray(subTokens(tokens, 0, index), t), subTokens(tokens, index));
        }
        ...
        public static List<string[]> getAllArgs(string[] all_args)
        {
            all_args = Program.addPara(all_args);
            List<string[]> ss = splitTokens(all_args, ",");
            if (ss.Count == 0)
                ss.Add(new string[0]);
            return ss;
        }
        ...
        static public string[] readCommond()
        {
            ...
            do
            {
                ...
                tokens = mergerArray(tokens, toTokens(buf));
                next = true;
        ...
        static public string[] processTokens(string[] tokens,ref Frame frame,out IExpression ret,ref bool end ,bool print = true)
        {
            ...
            while(true)
            {
                ...
                sentence = mergerArray(sentence, subTokens(tokens, 0, i));
                tokens = subTokens(tokens, i + 1);
                if (!paraMatchs(sentence, "{}"))
                {
                    sentence = mergerArray(sentence, ";");
                    continue;
                }
        ...
        static public IExpression processSentence(string[] sentence,ref Frame frame,ref bool end)
        {
            ...
            {
                Console.WriteLine("{0}", Tools.determine(subTokens(sentence, 1), frame));
            }
        ...
        static public string[] toTokens(string buf)
        {
            ...
            while (buf.Length > 0)
            {
                ...
                buf = buf.Substring(i);
                result = mergerArray(result, token);
            }
        ...
        static void Main(string[] args)
        {
            ...
            if (!__forcePath__ && args.Length == 0)
            {
                ...
                while (true)
                {
                    ...
                    try
                    {
                        tokens = mergerArray(tokens, readCommond());
            ...
            else
            {
                ...
                while (true)
                {
                    ...
                    try
                    {
                        tokens = mergerArray(tokens, readCommondInFile(sr));
                        tokens = processTokens(tokens, ref Tools.globalFrame, out nul,ref end);
                    }
                    catch (Exception e)
                    {
